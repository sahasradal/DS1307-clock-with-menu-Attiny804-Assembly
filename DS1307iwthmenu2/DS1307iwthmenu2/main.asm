...
; DS1307iwthmenu2.asm
;
; Created: 7/20/2021 2:52:18 PM
; Author : pappan

;code sucessfully working with ATTINY804 ,Hitachi1602 I2C LCD, ALIEXPRESS DS1307 RTC
;Only thing I was not able to identify was the clock displayed numeral + 1 than what i selected
;I select 01 for hour but the clock finaly displayed 02 for hour,this is same for all values
;Debugging didnt reveal anything. I finally subtracted 1 from the values selected which is 
;like a feild fix but the real reason remains unknown/mystery to me. 
;
;
...
rcall TWI_START
pos 0 , 0
lds SLAVE_REG,HOUR1stdigit
rcall DATA_WRITE
lds SLAVE_REG,HOUR2nddigit
rcall DATA_WRITE
lds SLAVE_REG,COLON1
rcall DATA_WRITE
lds SLAVE_REG,MINUTES1stdigit
rcall DATA_WRITE
lds SLAVE_REG,MINUTES2nddigit
rcall DATA_WRITE
lds SLAVE_REG,COLON2
rcall DATA_WRITE
lds SLAVE_REG,SECOND1stdigit
rcall DATA_WRITE
lds SLAVE_REG,SECOND2nddigit
rcall DATA_WRITE
lds SLAVE_REG,BLANK
rcall DATA_WRITE
pos 1,0
lds SLAVE_REG,DATE1stdigit
rcall DATA_WRITE
lds SLAVE_REG,DATE2nddigit
rcall DATA_WRITE
lds SLAVE_REG,SLASH1
rcall DATA_WRITE
lds SLAVE_REG,MONTH1stdigit
rcall DATA_WRITE
lds SLAVE_REG,MONTH2nddigit
rcall DATA_WRITE
lds SLAVE_REG,SLASH2
rcall DATA_WRITE
lds SLAVE_REG,YEAR1stdigit
rcall DATA_WRITE
lds SLAVE_REG,YEAR2nddigit
rcall DATA_WRITE
lds SLAVE_REG,SLASH3
rcall DATA_WRITE
lds SLAVE_REG,DAY1
rcall DATA_WRITE
lds SLAVE_REG,DAY2
rcall DATA_WRITE
lds SLAVE_REG,PERIOD
rcall DATA_WRITE
rcall TWI_STOP
ret
...
save_hour:
	millis 2000				; delay for visual needs
	subi r20,1				; I dont know why this works,if not subtracted the stored value is always 1 more than selected ,1 will become 2 etc
...
clockwrite:							; routine to copy adjusted time from SRAM to DS1307
	...
	rcall TWI_WRITE
	lds SLAVE_REG,minbcd			; write saved minutes
	rcall TWI_WRITE	
	lds SLAVE_REG,hourbcd			; write saved hours
	rcall TWI_WRITE			
	lds SLAVE_REG,weekbcd			; write from saved day of week 
	rcall TWI_WRITE
	lds SLAVE_REG,datebcd			; write to date register  
	rcall TWI_WRITE
	lds SLAVE_REG,monbcd			; write to month register
	rcall TWI_WRITE
	lds SLAVE_REG,yearbcd			; write to year register 
	rcall TWI_WRITE
	rcall TWI_STOP
